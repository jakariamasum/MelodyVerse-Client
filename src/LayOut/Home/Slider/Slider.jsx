  ...
  return (
    <div className="w-full h-[100vh] relative overflow-hidden border border-gray-300">
      {slides.map((slide, index) => (
        <div
          key={index}
          className={`absolute top-0 left-0 w-full h-full ${index === currentSlide ? 'opacity-100' : 'opacity-0'
            } transition-opacity duration-500`}
        >
          <img className="w-full h-full object-cover" src={slide} alt={`Image ${index + 1}`} />
          <div className="absolute inset-0 flex items-center justify-center flex-col">
            <p className=" text-3xl font bold text-orange-500">Unlock Your Musical Potential <br />
              Join Our Vibrant Music School Today!</p>
            <Link>
